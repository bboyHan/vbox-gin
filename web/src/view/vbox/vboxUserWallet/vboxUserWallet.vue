<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="充值数" prop="recharge">
             ...
             <el-input v-model.number="searchInfo.recharge" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="费率" prop="tariff">
            
             <el-input v-model.number="searchInfo.tariff" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="会员等级" prop="vipLevel" width="120" />
        <!-- <el-table-column align="left" label="操作">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateVboxUserWalletFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getVboxUserWalletList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteVboxUserWalletByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateVboxUserWalletFunc = async(row) => {
    const res = await findVboxUserWallet({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteVboxUserWalletFunc = async (row) => {
    const res = await deleteVboxUserWallet({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
