  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" min-width="120">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBdaChIndexDFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="detailShow" style="width: 800px" :draggable="true" lock-scroll :before-close="closeDetailShow" title="查看详情" destroy-on-close>
...
const onDelete = async() => {
      ...
      const res = await deleteBdaChIndexDByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBdaChIndexDFunc = async(row) => {
    const res = await findBdaChIndexD({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rebdaChIndexD
...
// 删除行
const deleteBdaChIndexDFunc = async (row) => {
    const res = await deleteBdaChIndexD({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBdaChIndexD(formData.value)
                  break
                case 'update':
                  res = await updateBdaChIndexD(formData.value)
                  break
                default:
                  res = await createBdaChIndexD(formData.value)
                  break
