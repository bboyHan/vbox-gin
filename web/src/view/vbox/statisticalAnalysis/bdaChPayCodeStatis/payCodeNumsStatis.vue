...
const pcOverview = async () => {
  ...
  // 渲染数据
  for (const moneyPart in data) {
    for (const opPart in data[moneyPart]) {
      for (const locPart in data[moneyPart][opPart]) {
        const waitCnt = data[moneyPart][opPart][locPart].waitCnt;
        const pendingCnt = data[moneyPart][opPart][locPart].pendingCnt;
...
const getTableData = async () => {
  const table = await getChannelPayCodeStatisByLocation({
...
const getProviceTableData = async () => {
  const table = await getChannelPayCodeStatisByLocation({
    page: page.value, pageSize: pageSize.value, location: location.value,
