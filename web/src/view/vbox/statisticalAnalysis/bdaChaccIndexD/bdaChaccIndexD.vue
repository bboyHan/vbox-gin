      ...
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="通道账户名:"  prop="acAccount" >
              <el-input v-model="formData.acAccount" :clearable="true"  placeholder="请输入通道账户名" />
            </el-form-item>
            <el-form-item label="账户备注:"  prop="acRemark" >
              <el-input v-model="formData.acRemark" :clearable="true"  placeholder="请输入账户备注" />
            </el-form-item>
            <el-form-item label="通道code:"  prop="channelCode" >
...
<script setup>
import {
  createBdaChaccIndexD,
  deleteBdaChaccIndexD,
...
const onDelete = async() => {
      ...
      const res = await deleteBdaChaccIndexDByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBdaChaccIndexDFunc = async(row) => {
...
// 删除行
const deleteBdaChaccIndexDFunc = async (row) => {
    const res = await deleteBdaChaccIndexD({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBdaChaccIndexD(formData.value)
                  break
                case 'update':
                  res = await updateBdaChaccIndexD(formData.value)
                  break
                default:
                  res = await createBdaChaccIndexD(formData.value)
                  break
