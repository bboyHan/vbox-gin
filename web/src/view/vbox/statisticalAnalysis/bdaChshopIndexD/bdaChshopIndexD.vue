    ...
    <el-dialog v-model="detailShow" style="width: 800px" lock-scroll :draggable="true" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
      <el-scrollbar height="550px">
        <el-descriptions column="1" border>
                ...
                <el-descriptions-item label="店铺备注">
                        {{ formData.shopRemark }}
                </el-descriptions-item>
...
const getTableData = async() => {
  const table = await getBdaChShopIndexDList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const updateBdaChShopIndexDFunc = async(row) => {
    ...
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rebdaChshopIndexD
