  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="状态开关" prop="status" width="140">
          <template #default="scope">
            <el-row :gutter="16">
              <el-col :span="10">
                <el-switch
    ...
    <!--  查询  -->
    <el-dialog v-model="dialogQueryFormVisible" :before-close="closeDialog" :title="'查询'"
               destroy-on-close>
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        <el-form-item label="token" prop="token">
          <el-input v-model="formData.token" type="textarea" :clearable="true" placeholder="请输入"/>
...
// 查询
const queryHisRecords = async (row) => {
  const res = await queryCAHisRecords({ID: row.ID})
  type.value = 'query'
  if (res.code === 0) {
    formData.value = res.data.revca
    dialogQueryFormVisible.value = true
  }
}
...
// query form
const dialogQueryFormVisible = ref(false)
