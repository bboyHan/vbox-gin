<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="通道编码ID" prop="channelCode" width="120" />
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateVboxChannelRateFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteVboxChannelRate,
  deleteVboxChannelRateByIds,
  updateVboxChannelRate,
  findVboxChannelRate,
  getVboxChannelRateList
...
const getTableData = async() => {
  const table = await getVboxChannelRateList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteVboxChannelRateByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateVboxChannelRateFunc = async(row) => {
    const res = await findVboxChannelRate({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteVboxChannelRateFunc = async (row) => {
    const res = await deleteVboxChannelRate({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
