  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateVboxBdaChaccIndexDFunc(scope.row)">变更</el-button>
...
<script>
export default {
  name: 'VboxBdaChaccIndexD'
}
</script>

<script setup>
import {
  createVboxBdaChaccIndexD,
  deleteVboxBdaChaccIndexD,
  deleteVboxBdaChaccIndexDByIds,
  updateVboxBdaChaccIndexD,
...
const getTableData = async() => {
  const table = await getVboxBdaChaccIndexDList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteVboxBdaChaccIndexDFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteVboxBdaChaccIndexDByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateVboxBdaChaccIndexDFunc = async(row) => {
    const res = await findVboxBdaChaccIndexD({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rebdaChaccD
...
// 删除行
const deleteVboxBdaChaccIndexDFunc = async (row) => {
    const res = await deleteVboxBdaChaccIndexD({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createVboxBdaChaccIndexD(formData.value)
                  break
                case 'update':
                  res = await updateVboxBdaChaccIndexD(formData.value)
                  break
                default:
                  res = await createVboxBdaChaccIndexD(formData.value)
                  break
