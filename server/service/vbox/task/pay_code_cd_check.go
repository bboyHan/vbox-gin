				...
				for _, pcDB := range pcDBList {
					...
					if pcDB.CodeStatus == 1 {
						...
						if accDB.DlyCntLimit > 0 {
							...
							if int(count) >= accDB.DlyCntLimit { // 如果日进单消费已经超了，不允许开启了，直接结束
								flag = true
								msgX := fmt.Sprintf(global.AccDlyCntLimitNotEnough, accDB.AcId, accDB.AcAccount, count, accDB.DlyCntLimit)

								global.GVA_LOG.Error("【DlyCntLimit】当前账号日进单消耗已经超限额...", zap.Any("msg", msgX), zap.Any("cnt", count), zap.Any("limit", accDB.DlyCntLimit))
								err = global.GVA_DB.Unscoped().Model(&vbox.ChannelAccount{}).Where("id = ?", accDB.ID).
									Update("sys_status", 0).Error
								global.GVA_LOG.Warn("当前账号日进单消耗已经超限额了，结束...", zap.Any("ac info", accDB))
							}
						}
						// 2.4 拉单限制
