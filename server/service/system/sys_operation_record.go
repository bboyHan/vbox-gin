...
func (operationRecordService *OperationRecordService) GetSysOperationRecordInfoSelf(info systemReq.SysOperationRecordSearch) (list interface{}, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// 创建db
	db := global.GVA_DB.Model(&system.SysOperationRecord{})
	...
	if len(info.Ids) != 0 {
		db = db.Where("user_id in ?", info.Ids)
	}
	err = db.Count(&total).Error
	if err != nil {
		return
