...
func (b *BaseApi) Register(c *gin.Context) {
	...
	if r.NickName == "" {
		r.NickName = rand_string.RandomLetter(8)
	}
...
// @Success  200   {object}  response.Response{data=systemRes.SysUserResponse,msg=string}  "用户注册账号,返回包括用户信息"
// @Router   /user/admin_register [post]
func (b *BaseApi) OwnerRegister(c *gin.Context) {
	var r systemReq.Register
	err := c.ShouldBindJSON(&r)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
	...
	err = db.Preload("DataAuthorityId").Where("parent_id = ?", userInfo.AuthorityId).First(&authority).Error

	fmt.Printf("auth info : %v", authority)
	r.AuthorityId = authority.AuthorityId
	err = utils.Verify(r, utils.RegisterVerify)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	...
	user := &system.SysUser{Username: r.Username,
		...
		Phone:       r.Phone,
		Email:       r.Email,
		ParentId:    int(userInfo.BaseClaims.ID)}
	userReturn, err := userService.Register(*user)
	if err != nil {
		global.GVA_LOG.Error("注册失败!", zap.Error(err))
		response.FailWithDetailed(systemRes.SysUserResponse{User: userReturn}, "注册失败", c)
